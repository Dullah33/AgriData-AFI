...
<div class="bg-white rounded-xl shadow-sm p-6 max-w-xl">
    <div class="flex items-center gap-3 mb-6">
        ...
        <h2 class="text-lg font-semibold text-gray-800">Edit Pelatihan: {{ $pelatihan->judul }}</h2>
    </div>
    ...
    <form method="POST" action="{{ route('penyuluh.pelatihan.update', $pelatihan) }}" class="space-y-4">
        @csrf @method('PUT')

        <div>
            <label class="block text-sm font-medium text-gray-700 mb-1">Judul Kegiatan <span class="text-red-500">*</span></label>
            <input type="text" name="judul" value="{{ old('judul', $pelatihan->judul) }}"
        ...
        <div>
            ...
            <textarea name="deskripsi" rows="3"
                      class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-teal-500">{{ old('deskripsi', $pelatihan->deskripsi) }}</textarea>
        </div>
        ...
        <div class="grid grid-cols-2 gap-4">
            ...
            <div>
                ...
                <input type="date" name="tanggal_pelaksanaan"
                       value="{{ old('tanggal_pelaksanaan', $pelatihan->tanggal_pelaksanaan->format('Y-m-d')) }}"
                       class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-teal-500">
            </div>
        </div>
        ...
        <div class="grid grid-cols-2 gap-4">
            <div>
                ...
                <input type="number" min="0" name="jumlah_peserta" value="{{ old('jumlah_peserta', $pelatihan->jumlah_peserta) }}"
                       class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-teal-500">
            </div>
            <div>
                ...
                <select name="status"
                        class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-teal-500">
                    @foreach (['terjadwal' => 'Terjadwal', 'selesai' => 'Selesai', 'batal' => 'Dibatalkan'] as $val => $label)
                        <option value="{{ $val }}" {{ old('status', $pelatihan->status) === $val ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            </div>
        </div>
